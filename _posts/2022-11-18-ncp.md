...
하이퍼바이저는 호스트 컴퓨터로 다수의 운영체제를 동시에 실행하기 위하는 논리적 플랫폼으로서 게스트 운영체제와 그 운영체제에서 구동되는 프로그램을 실제 물리적 장치에서 분리하는 프로세스를 가리킨다. 주로 소프트웨어 형태로 구현되지만, 임베디드된 하드웨어의 형태로 제작되기도 한다. 하이퍼바이저는 '가상화 머신 모니터(virtual machine monitor)' 또는 '가상화 머신 매니저(virtual machine manager)'로 불리기도 하며, 준말인 'VMM'로도 불린다.
...
과거에는 기본적으로 PC 1대에는 1개의 OS 만이 구동할 수 있었다. 하지만 PC의 리소스를 효율적으로 사용하기 위해 1대의 PC 에서 여려개의 Guest OS를 사용하기 위해서 가상화 시스템이란걸 사용하기 시작했다.<br>
**1세대** 가상화 시스템은 APP위에 마치 PC 와 같은 시스템을 구현시키기위해 에뮬레이터를 이용하였고, Guest OS 는 이러한 에뮬레이터를 통해 가상화 된 H/W 위에 설치되어서 PC의 H/W 와 신호를주고받을 수 없었고 그렇다보니 Guest OS의 속도가 느리고 리소스의 낭비가 상대적으로 크다<br>
**2세대**에 들어와서는 **하이퍼바이저**라는 플랫폼이 등장하였다. Guest OS는 기존의 APP위에서 에뮬레이터를 통해 가상화된 H/W 가 아닌 PC의 H/W 와 하이퍼바이저를 통해 직접 신호를 주고받을수 있게되었다. 그로인해 Guest OS의 작동 속도가 이전에 비하면 매우 빨라졌고 PC의 리소스 이전보다 효율적으로 사용할 수 있게되었다.<br>
**3세대(HVM)**의 경우에는 하이퍼바이저 역시 사용하지 않고 H/W 가 직접 가상화를 지원하여 Host OS 위에 바로 Guest OS 를 설치하고 Guest OS 가 별도의 플랫폼의 도움 없이도 바로 H/W 와 신호를 주고 받을 수 있게되었다 
