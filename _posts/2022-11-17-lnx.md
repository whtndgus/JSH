...
### 이해 | 정리
<br>
마크다운이란 마크업 언어의 일종이라고 한다, 여기서 말하는 마크업 언어란 태그 등을 이용하여 문서나 데이터의 구조를 명기하는 것을 말한다고 한다, 쉽게 말하면 우리가 웹브라우저를 통해 보는 화면을 만드는 언어라고 볼 수 있다. 대표적인 마크업 언어로는 HTML이 많이 사용되고 있는데 HTML의 경우 태그 구조로 되어 있어 텍스트를 작성할 때 그에 해당하는 태그를 명시할 필요가 있다.(예: <"div">빛이 있으라.</"div'> ) 이러한 HTML의 태그 구조를 사용하지 않고 일반적인 텍스트 편집기로 문서를 작성하는 것만으로도 화면을 구성할 수 있도록 만들어진 것이 마크다운(Markdown)이다. 마크다운의 문법은 기본적인 편집기인 메모장만으로도 손쉽게 문서를 작성하여 화면 구성할 수 있기에 접근성 매우 우월하다고 하지만 기존의 HTML의 모든 문법을 대신하지는 못하고, 명확한 표준이 없어 사용자마다 문법이 다를 수 있다.
